...
object Migrations {
    ...
    val VERSION_10_11: Migration =
        object : Migration(10, 11) {
            override fun migrate(database: SupportSQLiteDatabase) {
                ...
                // Using runCatching to avoid crash if columns already exist
                kotlin.runCatching { database.execSQL("ALTER TABLE games ADD COLUMN year INTEGER") }
                kotlin.runCatching { database.execSQL("ALTER TABLE games ADD COLUMN genre TEXT") }
                kotlin.runCatching { database.execSQL("ALTER TABLE games ADD COLUMN description TEXT") }
                kotlin.runCatching { database.execSQL("ALTER TABLE games ADD COLUMN publisher TEXT") }
            }
        }
    ...
    val VERSION_11_12: Migration =
        object : Migration(11, 12) {
            override fun migrate(database: SupportSQLiteDatabase) {
                kotlin.runCatching { database.execSQL("ALTER TABLE games ADD COLUMN isUserLocked INTEGER NOT NULL DEFAULT 0") }
            }
        }
}
