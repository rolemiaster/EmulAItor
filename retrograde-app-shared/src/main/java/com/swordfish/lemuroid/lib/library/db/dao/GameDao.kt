...
interface GameDao {
    ...
    @Query("SELECT * FROM games WHERE lastPlayedAt IS NULL ORDER BY title ASC LIMIT :limit")
    ...
    fun selectHomeGames(): PagingSource<Int, Game>
    ...
    suspend fun selectTotalCount(): Int
    ...
    @Query(
        """
        SELECT * FROM games
        WHERE isUserLocked = 0
        AND systemId != 'unknown'
        AND (
            developer IS NULL OR coverFrontUrl IS NULL OR year IS NULL OR genre IS NULL OR
            description IS NULL OR publisher IS NULL
        )
        ORDER BY lastIndexedAt DESC
        LIMIT :limit
        """,
    )
    suspend fun selectGamesNeedingMetadataEnrichment(limit: Int): List<Game>
