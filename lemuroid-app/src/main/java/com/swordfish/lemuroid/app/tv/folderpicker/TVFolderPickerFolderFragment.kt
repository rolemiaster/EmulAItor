...
class TVFolderPickerFolderFragment : GuidedStepSupportFragment() {
    ...
    override fun onGuidedActionClicked(action: GuidedAction) {
        when (action.id) {
            ACTION_CHOOSE -> {
                // Validate if we can actually read this directory
                if (directory.listFiles() == null) {
                    android.widget.Toast.makeText(requireContext(), R.string.tv_folder_error_access, android.widget.Toast.LENGTH_LONG).show()
                    return
                }
