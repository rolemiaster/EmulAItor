...
import com.swordfish.lemuroid.app.shared.license.openLicensePurchasePageAndFinish
import com.swordfish.lemuroid.app.shared.settings.SettingsInteractor
import com.swordfish.lemuroid.common.coroutines.safeLaunch
import com.swordfish.lemuroid.ext.feature.license.LicenseGate
import com.swordfish.lemuroid.ext.feature.license.LicenseGateResult
import com.swordfish.lemuroid.ext.feature.review.ReviewManager
import com.swordfish.lemuroid.ext.feature.update.UpdateManager
...
import com.swordfish.lemuroid.lib.updates.AppUpdateNotice
...
class MainActivity : RetrogradeComponentActivity(), BusyActivity {
    ...
    private var legacyStoragePermissionRequestInFlight = false
    private var legacyStoragePermissionRequested = false
    ...
    override fun onResume() {
        ...
        val repairedStoragePreferences = SharedPreferencesHelper.repairInvalidSAFUri(applicationContext)

        // V10 FIX: Fallback for broken SAF on some devices (Android 10/11)
        ensureLegacyStoragePermissionsIfNeeded()

        if (repairedStoragePreferences) {
    ...
    override fun onPause() {
        super.onPause()
        if (::musicPlayer.isInitialized && musicPlayer.isPlaying.value) {
            musicPlayer.pauseTemporarily()
        }
    }

    private fun ensureDefaultFolderConfigured() {
        if (SharedPreferencesHelper.getSAFUri(applicationContext) != null) return
        if (SharedPreferencesHelper.getLegacyFolderPath(applicationContext) != null) return

        val defaultFile = java.io.File(android.os.Environment.getExternalStorageDirectory(), "EmulAI_Roms")
        val defaultFolderReady = (defaultFile.exists() || defaultFile.mkdirs()) &&
            defaultFile.isDirectory && defaultFile.canRead() && defaultFile.canWrite()

        if (!defaultFolderReady) return

        val legacyKey = getString(com.swordfish.lemuroid.lib.R.string.pref_key_legacy_external_folder)
        SharedPreferencesHelper.getLegacySharedPreferences(applicationContext).edit()
            .putString(legacyKey, defaultFile.absolutePath)
            .apply()
    ...
    private fun requestLegacyStoragePermission() {
        if (legacyStoragePermissionRequestInFlight || legacyStoragePermissionRequested) return

        legacyStoragePermissionRequestInFlight = true
        legacyStoragePermissionRequested = true
    ...
    private fun MainScreen(navController: NavHostController) {
        AppTheme {
            ...
            val updateNotice = remember { mutableStateOf<AppUpdateNotice?>(null) }

            LaunchedEffect(Unit) {
                updateNotice.value = updateManager.checkForUpdate(applicationContext)
            }
            ...
            val mainUIState = baseUIState.copy(
                ...
                updateAvailable = updateNotice.value != null,
                onUpdateClick = {
                    updateNotice.value?.let { updateManager.launchUpdate(this@MainActivity, it) }
                },
            )
            ...
            BackgroundWithOverlay {
                ...
                bottomBar = {
                    MainNavigationBar(
                        currentRoute = currentRoute,
                        navController = navController,
                        onSearchSelected = { mainViewModel.changeQueryString("") },
                    )
                },
            ) { padding ->
                NavHost(
            ...
            gameToEdit?.let { game ->
                GameEditDialog(
                    game = game,
                    onSearchOnline = { query -> gameMetadataProvider.searchByName(query) },
