...
class TVHomeFragment : BrowseSupportFragment() {
        ...
        onItemViewClickedListener =
            OnItemViewClickedListener { _, item, _, _ ->
                when (item) {
                    ...
                    is TVSetting -> {
                        when (item.type) {
                            TVSettingType.METADATA_ENRICHMENT -> Unit
                            TVSettingType.HD_MODE_ON,
                            TVSettingType.HD_MODE_OFF,
                            -> toggleHdMode()
    ...
    private fun update(viewState: TVHomeViewModel.HomeViewState) {
        ...
        findAdapterById<ArrayObjectAdapter>(SETTINGS_ADAPTER)?.setItems(
            buildSettingsRowItems(
                ...
                viewState.metadataEnrichmentInProgress,
                viewState.hdModeEnabled,
            ),
    ...
    private fun recreateAdapter(
        ...
        val catalogItems = ArrayObjectAdapter(SettingPresenter(cardSize, settingsCardPadding))
        catalogItems.add(TVSetting(TVSettingType.CATALOG, true))
        val catalogTitle = resources.getString(R.string.title_catalog)
        result.add(ListRow(HeaderItem(CATALOG_ADAPTER, catalogTitle), catalogItems))
        ...
        settingsItems.setItems(
            buildSettingsRowItems(
                ...
                metadataEnrichmentInProgress = false,
                hdModeEnabled = false,
            ),
            LEANBACK_SETTING_DIFF_CALLBACK,
        )
    ...
    private fun buildSettingsRowItems(
        ...
        return mutableListOf<TVSetting>().apply {
            if (metadataEnrichmentInProgress && !indexInProgress) {
                add(TVSetting(TVSettingType.METADATA_ENRICHMENT, false))
            }
            add(
                TVSetting(
                    if (hdModeEnabled) {
    ...
    private fun toggleHdMode() {
        val preferences = SharedPreferencesHelper.getSharedPreferences(requireContext())
        val key = getString(R.string.pref_key_hd_mode)
        preferences.edit { putBoolean(key, !preferences.getBoolean(key, false)) }
    }
