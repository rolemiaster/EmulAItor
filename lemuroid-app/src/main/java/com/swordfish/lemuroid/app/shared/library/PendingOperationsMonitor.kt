...
class PendingOperationsMonitor(private val appContext: Context) {
    enum class Operation(val uniqueId: String, val isPeriodic: Boolean) {
        ...
        METADATA_ENRICHMENT(LibraryIndexScheduler.LIBRARY_METADATA_ENRICHMENT_WORK_ID, false),
    ...
    fun anyOperationInProgress(): Flow<Boolean> {
        return operationsInProgress(
            ...
            Operation.SAVES_SYNC_PERIODIC,
            Operation.SAVES_SYNC_ONE_SHOT,
        )
    }
    ...
    fun isMetadataEnrichmentInProgress(): Flow<Boolean> {
        return operationsInProgress(Operation.METADATA_ENRICHMENT)
    }
