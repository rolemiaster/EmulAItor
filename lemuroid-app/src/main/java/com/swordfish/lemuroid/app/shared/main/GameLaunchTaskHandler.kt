...
class GameLaunchTaskHandler(
    ...
    private val accessManager: com.swordfish.lemuroid.ext.feature.access.AccessManager,
) {
    suspend fun checkAccessAndStart(activity: Activity): Boolean {
        if (!accessManager.hasAccess(activity)) {
            val intent = Intent(activity, com.swordfish.lemuroid.app.mobile.feature.paywall.PaywallActivity::class.java)
            activity.startActivity(intent)
            return false
        }
        handleGameStart(activity.applicationContext)
        return true
    }
    ...
    suspend fun handleGameFinish(
        ...
        if (resultCode == BaseGameActivity.RESULT_RESTART) {
            handleGameRestart(activity, data)
            return
        }
    ...
    private suspend fun handleGameRestart(
        activity: Activity,
        data: Intent?,
    ) {
        val game = data?.extras?.getSerializable(BaseGameActivity.PLAY_GAME_RESULT_GAME) as? Game ?: return
        val systemCoreConfig =
            data.extras?.getSerializable(BaseGameActivity.PLAY_GAME_RESULT_SYSTEM_CORE_CONFIG) as? SystemCoreConfig
                ?: return
        val useLeanback = data.extras?.getBoolean(BaseGameActivity.PLAY_GAME_RESULT_LEANBACK) == true

        delay(activity.animationDuration().toLong() + 100L)
        if (!checkAccessAndStart(activity)) {
            return
        }
        BaseGameActivity.launchGame(activity, systemCoreConfig, game, false, useLeanback)
    }
    ...
    private suspend fun handleSuccessfulGameFinish(
        ...
        accessManager.addTrialTime(duration)
        if (enableRatingFlow) {
