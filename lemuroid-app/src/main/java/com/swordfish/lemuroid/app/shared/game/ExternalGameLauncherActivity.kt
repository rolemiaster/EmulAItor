...
class ExternalGameLauncherActivity : ImmersiveActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            ...
            lifecycleScope.launch {
                ...
                try {
                    when (val result = licenseGate.verify(this@ExternalGameLauncherActivity)) {
                        LicenseGateResult.Allowed -> loadGame(gameId)
                        is LicenseGateResult.Blocked -> displayLicenseError(result.message)
                    }
                } catch (e: Throwable) {
                    displayErrorMessage()
                }
    ...
    private fun displayLicenseError(message: String) {
        android.app.AlertDialog.Builder(this)
            .setTitle(getString(R.string.license_not_verified_title))
            .setMessage(message)
